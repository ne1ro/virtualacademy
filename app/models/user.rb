class User < ActiveRecord::Base
  ...
  validates :nickname, presence: true, format: {with: /\A[a-z0-9]*\z/, message: 'only downcase letters and numbers allowed'},\
    length: {in: 4..36, message: 'should have at least 4 symbols and 36 as max'}, uniqueness: true

  validates :location, format: {with: /\A[a-zA-Zа-яА-Я0-9, \.]*\z/, message: 'should be only letters and symbols with comma separated'}, \
  length: {in: 2..250, tokenizer: lambda { |str| str.scan(/\w+/)}, too_short: "must have at least %{count} words", too_long: "must have at most %{count} words"}
  ...
  validates :fullname, presence: true, length: {in: 2..10, tokenizer: lambda { |str| str.scan(/\w+/)},\
